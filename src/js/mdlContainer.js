...
var mdlContainers = {
    "generateHtml" : innerContent => {
        return _rw.newDocument(
            // Head Scripts
            _rw.include("https://ajax.googleapis.com/ajax/libs/jquery/3.4.1/jquery.min.js") + // Jquery
            _rw.include("https://code.getmdl.io/1.3.0/material.min.js", true) + // mdl, defer load
            _rw.include("https://cdnjs.cloudflare.com/ajax/libs/dialog-polyfill/0.4.2/dialog-polyfill.min.js") +
            _rw.materialDropdown() +
            _rw.style("https://fonts.googleapis.com/css2?family=Roboto:ital,wght@0,100;0,300;0,400;0,500;0,700;0,900;1,100;1,300;1,400;1,500;1,700;1,900&display=swap") + // roboto
            _rw.style("https://fonts.googleapis.com/icon?family=Material+Icons") +
            _rw.style(`https://code.getmdl.io/1.3.0/material.`+ (()=>{
                if (rw.config.colTheme){
                    return rw.config.colTheme;
                } else {
                    return "blue-indigo"; // default theme
                }
            })() +`.min.css`) // load theme
            , // end head param
            innerContent // body
...
var _rw = { // content generation for mdl elements

    "newDocument" : (head,body) => {
        // Generate a new document
        return `<!DOCTYPE html>
<html><head>${head}</head><body>${body}</body></html>`;
    },

    "include" : (scriptURL, defer)=>{ // add script tag - setting defer to true will defer the script load
        return `<script ${(defer ? "defer" : "")} src="${scriptURL}"></script>`;
    },

    "style" : url=>{ // add CSS file
        return `<link rel="stylesheet" href="${url}">`;
    },

    "materialDropdown" : ()=>{ // Return code for material dropdown
        return `<!-- Material dropdown - MIT License, Copyright (c) 2016 CreativeIT https://github.com/CreativeIT/getmdl-select/blob/master/LICENSE.txt -->
        <style>
