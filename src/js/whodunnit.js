...
rw.whodunnit = {
    ...
    "mouseHighlighter" : () => {
        ...
        $.getJSON("https://en.wikipedia.org/w/api.php?action=parse&page="+ encodeURIComponent(mw.config.get("wgRelevantPageName")) +"&prop=text&format=json&formatversion=2", r=>{
    ...
    "processNext" : ()=> {
        ...
        $.getJSON("https://en.wikipedia.org/w/api.php?action=parse&format=json&oldid="+ revID, r=>{ // Get revision content
            if (!r.parse.text["*"].replace(/[^\w\s!?]/g, "").includes(html)) {
                if (rw.whodunnit.i == 0) {
                    ...
                    redirect("https://en.wikipedia.org/w/index.php?diff="+ cprID +"&oldid="+ cpprID +"&diffmode=source"); // go
                    // we done
                }
            } else {
                // Let's continue
