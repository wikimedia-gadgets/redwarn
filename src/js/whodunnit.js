...
rw.whodunnit = {
    ...
    "mouseHighlighter" : () => {
        ...
        $.getJSON(WIKICWD+"/w/api.php?action=parse&page="+ encodeURIComponent(mw.config.get("wgRelevantPageName")) +"&prop=text&format=json&formatversion=2", r=>{
    ...
    "locate" : htmlIn=> { // Locate first instance of this html 
        ...
        $.getJSON(WIKICWD+"/w/api.php?action=query&prop=revisions&titles="+ encodeURIComponent(name) +"&rvlimit=500&rvprop=ids%7Cuser%7Ctimestamp&format=json", r=>{
    ...
    "processNext" : ()=> {
        ...
        $.getJSON(WIKICWD+"/w/api.php?action=parse&format=json&oldid="+ revID, r=>{ // Get revision content
            if (!r.parse.text["*"].replace(/[^\w\s!?]/g, "").includes(html)) {
                if (rw.whodunnit.i == 0) {
                    ...
                    redirect(WIKICWD+"/w/index.php?diff="+ cprID +"&oldid="+ cpprID +"&diffmode=source"); // go
                    // we done
                }
            } else {
                // Let's continue
